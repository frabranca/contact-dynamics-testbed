...
// compile with:
// g++ -o motion_with_control motion_with_control.cpp -llcm -lfranka -lPocoUtil -lPocoXML -lPocoJSON -lPocoNet -lPocoFoundation
...
// NAMESPACE WITH CONTROLLER AND DQ FILTER ------------------------------------------------------------------------------------------------
namespace {
...
class Controller {
 public:
  ...
  inline franka::Torques step(const franka::RobotState& state_read, const exlcm::command* msg) {
    updateDQFilter(state_read);
  ...
  void updateDQFilter(const franka::RobotState& state_read) {
    for (size_t i = 0; i < 7; i++) {
      dq_buffer_.get()[dq_current_filter_position_ * 7 + i] = state_read.dq[i];
    }
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    lcm::LCM lcm;
    exlcm::command *msg; // with or without * ??
    exlcm::state my_data;
      
    robot.control(
        [&](const franka::RobotState& robot_state, franka::Duration) -> franka::Torques {    
        
        //franka::RobotState state_read = robot.readOnce();
        // UPDATE STATE ON LCM
        for (int i=0; i<7; i++){
          my_data.q[i] = robot_state.q[i];
          my_data.q_d[i] = robot_state.q_d[i];
          my_data.dq[i] = robot_state.dq[i];
          my_data.dq_d[i] = robot_state.dq_d[i];
          my_data.ddq_d[i] = robot_state.ddq_d[i];
          my_data.tau_J[i] = robot_state.tau_J[i];
          my_data.tau_J_d[i] = robot_state.tau_J_d[i];
          my_data.dtau_J[i] = robot_state.dtau_J[i];    		
        }
            
        // PUBLISH LCM MESSAGE
        lcm.publish("STATE", &my_data); 
            
        return controller.step(robot_state, msg);
        },
